...
def bsGetLevels():
    return [bs.Level('Wizard War Persian',
                     displayName='${GAME}',
                     gameType=WizardWar,
                     settings={},
...
class WWBomb(bs.Bomb):
    ...
    def _handleImpact(self,m):
        ...
        if self.getActivity().settings["فعال کردن ترکیدن توپ ها"]:
...
class WizardWar(bs.TeamGameActivity):
    ...
    def getSettings(cls, sessionType):
        return [("Epic Mode", {'default': False}),
                ...
                ("Enable Picking Up", {'default': True}),
    ...
    def onBegin(self):
        ...
        for player in self.players:
            player.actor.connectControlsToPlayer(enableBomb=False, enablePickUp = s["فعال کردن بلند کردن اجسام"], enableRun = s["فعال کردن دویدن"], enableJump = s["فعال کردن پریدن"])
    ...
    def shootBomb(self, spaz):
        if spaz._orbNum >= self.settings["تعداد توپ ها"]: return
        spaz._orbNum += 1
    ...
    def handleMessage(self, m):
        if isinstance(m, bs.PlayerSpazDeathMessage):
            ...
            bs.screenMessage(str(m.spaz.getPlayer().getName()) + " !مردید", m.spaz.getPlayer().color, top=True)
